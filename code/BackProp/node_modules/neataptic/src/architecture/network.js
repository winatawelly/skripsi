...
Network.prototype = {
  ...
  activate: function (input, training) {
    ...
    for (var i = 0; i < this.nodes.length; i++) { 
      if (this.nodes[i].type === 'input') {
        this.nodes[i].activate(input[input.length-1-i]);
      } 
    }

    for (var i = 0; i < this.nodes.length; i++) {
      if (this.nodes[i].type === 'hidden') {
    ...
    for (var i = 0; i < this.nodes.length; i++) {
      if (this.nodes[i].type === 'output') {
        var activation = this.nodes[i].activate();
        //console.log(this.nodes[i].key + ' '+activation)
   ...
   */
  train: function (set, options , propagate, showOutput = false) {
    if (set[0].input.length !== this.input || set[0].output.length !== this.output) {
      console.log("Network input =",this.input+" output = "+this.output)
      console.log("data input =",set[0].input.length+" output = "+set[0].output.length)
    ...
    while (error > targetError && (options.iterations === 0 || iteration < options.iterations)) {
      ...
      if (options.crossValidate) {
        this._trainSet(trainSet, batchSize, currentRate, momentum, cost , propagate , showOutput);
        if (options.clear) this.clear();
        error = this.test(testSet, cost).error;
        if (options.clear) this.clear();
      } else {
        error = this._trainSet(set, batchSize, currentRate, momentum, cost , propagate , showOutput);
        if (options.clear) this.clear();
      }
    ...
    console.log("Total time = ",Math.floor(((Date.now() - start) / 1000)));
   ...
   */
  _trainSet: function (set, batchSize, currentRate, momentum, costFunction , propagate , showOutput = false) {
    //console.log(set[0].output);
    var errorSum = 0;
    let correct = 0;
    let win = 0;
    ...
    if(!propagate){
      ...
      console.log("Prediction correct = "+correct+" ("+correct/set.length*100+")");
      console.log("winner correct = "+win+" ("+win/set.length*100+")")
