...
Network.prototype = {
   ...
   */
  train: function (set, options , propagate) {
    if (set[0].input.length !== this.input || set[0].output.length !== this.output) {
      console.log(this.input)
      console.log(set[0].input.length);
      console.log(set[0].output.length);
    ...
    while (error > targetError && (options.iterations === 0 || iteration < options.iterations)) {
      ...
      if (options.crossValidate) {
        this._trainSet(trainSet, batchSize, currentRate, momentum, cost , propagate);
        if (options.clear) this.clear();
        error = this.test(testSet, cost).error;
        if (options.clear) this.clear();
      } else {
        error = this._trainSet(set, batchSize, currentRate, momentum, cost , propagate);
        if (options.clear) this.clear();
      }
   ...
   */
  _trainSet: function (set, batchSize, currentRate, momentum, costFunction , propagate) {
